...
{
    cog_unit idx = key % COG_MAP_SIZE; //TODO:Improve.
...
void* cog_map_get_recurse(cog_map_elem* elem, cog_uint key)
{
    ...
    else
    {
        ...
        else
        {
            ...
            return cog_map_get_recurse(map, elem->next);
        }
    }
}
...
{
    cog_unit idx = key % COG_MAP_SIZE; //TODO:Improve.
    cog_elem* elem = map->elems[idx];
    if(elem==0)
    {
        //First elem in linked list
        cog_elem* newelem = (cog_map_elem*)cog_malloc(sizeof(cog_map_elem));
        newelem->key = key;
        newelem->data = data;
        newelem->next = 0;
